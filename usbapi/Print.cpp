	...
	if (OpenPrinter(szprintername, &phandle, NULL))
	{
		...
		if (printer_info != NULL)
		{
			if (GetPrinter(phandle, 2, (LPBYTE)printer_info, dmsize, &dmsize))
			{
				...
				devmode.dmPublic.dmDuplex = g_PrintSettingsData.m_duplexPrint; //DUPLEX， DMDUP_VERTICAL: 长边 DMDUP_HORIZONTAL，短边
		...
		if (printer_info != NULL)
		{
			if (GetPrinter(phandle, 2, (LPBYTE)printer_info, dmsize, &dmsize))
			{
				...
				*ptr_density = *ptr_density - 4;
				...
				inputDevmode.dmPublic.dmDuplex = *ptr_duplexPrint; //DUPLEX， DMDUP_VERTICAL: 长边 DMDUP_HORIZONTAL，短边
